...
export class YoutubeTwitchChatToggleWorker {
  ...
  private render() {
    ...
    wrapper.className = 'yt-twitch-chat-toggle-wrapper';

    // Create YouTube icon container
    const youtubeIcon = document.createElement('div');
    youtubeIcon.className = `yt-twitch-chat-site-icon youtube-icon-${this.channelName}`;

    const youtubeSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    youtubeSvg.setAttribute('width', '20');
    youtubeSvg.setAttribute('height', '20');
    youtubeSvg.setAttribute('viewBox', '0 0 24 24');
    youtubeSvg.setAttribute('fill', 'currentColor');
    ...
    // Create Twitch icon container
    const twitchIcon = document.createElement('div');
    twitchIcon.className = `yt-twitch-chat-site-icon twitch-icon-${this.channelName}`;

    const twitchSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    twitchSvg.setAttribute('width', '20');
    twitchSvg.setAttribute('height', '20');
    twitchSvg.setAttribute('viewBox', '0 0 24 24');
    twitchSvg.setAttribute('fill', 'currentColor');

    const twitchPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    twitchPath.setAttribute(
      'd',
    ...
    twitchSvg.appendChild(twitchPath);
    twitchIcon.appendChild(twitchSvg);

    // Append icons to wrapper
    wrapper.appendChild(youtubeIcon);
    wrapper.appendChild(twitchIcon);
